...
public class CoppetecStrutsParameterLogicImpl extends CoppetecStrutsParameterLogic {
	...
	protected String handleGetOnlineHelpValue() {
		...
		if ("password".equals(getWidgetType())) {
			buffer.append("Este é um campo de senha, ele não exibirá os dados digitados, ").append(
					"cada caractere será mascarado usando um asterisco.");
			buffer.append(crlf);
		}
	...
	protected java.util.Collection handleGetValidatorTypes() {
		...
		if (type != null) {
			...
			else if (isValidatorInteger() && !isPlainText())
				validatorTypesList.add("integer");
			else if (isValidatorLong())
				validatorTypesList.add("long");
			else if (!isMoney() && isValidatorFloat())
			...
			if (isRangeFormat) {
				if (isValidatorInteger() || isValidatorShort() || isValidatorLong())
					validatorTypesList.add("intRange");
				if (!isMoney() && isValidatorFloat())
		...
		if (isGenericValidator())
			validatorTypesList.add("genericValidator");
	...
	protected boolean handleIsSelectable() {
		if (super.handleIsSelectable())
			return true;
		if (isMultibox() && getMultiboxPropertyName() == null)
			return true;
		return false;
	}
	...
	/**
	 * @return <code>true</code> if the type of this field is a character,
