        ...
        extends EJBSessionFacadeLogic
{
    ...
    protected java.lang.String handleGetTransactionType()
    {
		...
		protected boolean handleValidateSharedDependencies() {
		if(findTaggedValue("@andromda.services.shared") != null) {
			for (Iterator iterator = getSourceDependencies().iterator(); iterator.hasNext();) {
				DependencyFacade dependency = (DependencyFacade) iterator.next();
				if(dependency.getTargetElement().hasStereotype("Entity")) {
					return false;
				}
			}
		}
